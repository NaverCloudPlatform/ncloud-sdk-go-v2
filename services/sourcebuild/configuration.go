...
func NewConfiguration(region string, apiKeys ...*ncloud.APIKey) *ncloud.Configuration {
	cfg := &ncloud.Configuration{
		BasePath:      "https://sourcebuild.apigw.ntruss.com/api/v1",
		DefaultHeader: make(map[string]string),
		UserAgent:     "sourcebuild/1.0.0/go",
	}
	if len(apiKeys) > 0 {
	...
	if os.Getenv("NCLOUD_API_GW") != "" {
		...
		cfg.BasePath = strings.Replace(cfg.BasePath, "fin-ncloud", "sourcebuild", 1)
		cfg.BasePath = strings.Replace(cfg.BasePath, "ncloud", "sourcebuild", 1)
	}
	switch region {
	case "SGN":
