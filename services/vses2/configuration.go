...
func NewConfiguration(region string, apiKeys ...*ncloud.APIKey) *ncloud.Configuration {
	cfg := &ncloud.Configuration{
		BasePath:      "https://vpcsearchengine.apigw.ntruss.com/api/v2",
	...
	switch region {
	case "KR": // 민간, 공공-수도권
		cfg.BasePath = fmt.Sprintf("%s/api/v2", ncloudApiGw)
	case "FKR": // 금융
		cfg.BasePath = fmt.Sprintf("%s/api/v2", ncloudApiGw)
	default: // 민간-싱가폴, 공공-남부권
		cfg.BasePath = fmt.Sprintf("%s/api/%s-v2", ncloudApiGw, strings.ToLower(region))
	}
	return cfg
}
